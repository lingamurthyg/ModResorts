...
public class WeatherServlet extends HttpServlet {
  ...
  public void destroy() {
    ...
    // Close Secrets Manager client to prevent resource leaks
  ...
  protected void doGet(HttpServletRequest request,
    ...
    // Retrieve API key from AWS Secrets Manager (cloud-native secret management)
  ...
  /**
   * Retrieve Weather API key from AWS Secrets Manager with fallback to environment variable
   */
  private String getWeatherApiKey() {
    // Try AWS Secrets Manager first (cloud-native approach)
    if (secretsManagerClient != null) {
      try {
        ...
        if (secret != null && !secret.isEmpty()) {
          logger.info("Successfully retrieved API key from AWS Secrets Manager");
          return secret;
        }
      } catch (Exception e) {
